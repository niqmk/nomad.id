...
@interface Home_Controller() <UITextFieldDelegate>
@end
typedef NS_ENUM(NSInteger, Home_Status) {
    NoneHomeStatusCode,
    SearchHomeStatusCode
};
...
- (void)didHomeViewSliderSelected:(Slider_Model *)slider_model {
    if([Global_Controller isNotNull:slider_model.link_url]) {
        [Global_Controller openWeb:slider_model.link_url];
    }else {
        [self openDetail:slider_model];
    }
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    home_status = NoneHomeStatusCode;
    init = true;
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if(home_status == NoneHomeStatusCode) {
        [Global_Controller setDefaultBacgroundWithLogo:self.navigationController];
    }else if(home_status == SearchHomeStatusCode) {
...
- (IBAction)doSearch:(id)sender {
    home_status = SearchHomeStatusCode;
    [Global_Controller removeAllViews:self.navigationController];
...
- (void)setMenuLayout {
    ...
    menu_list = [App_Controller getHomeList];
    float max_width = 0;
    NSString *first_key;
...
- (void)setLayout {
    ...
    for(NSString *each in menu_list) {
        UIView *layout = [Layout_Controller getLayout:each
                                                Width:size.width
                                             Delegate:self];
